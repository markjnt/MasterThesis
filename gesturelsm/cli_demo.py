...
class BaseTrainer(object):
    def __init__(self, args, cfg, wav_fp: str, textgrid_fp: str):
        ...
        # Copy provided files instead of generating them
        self.audio_path = tmp_dir + "/tmp.wav"
        self.textgrid_path = tmp_dir + "/tmp.TextGrid"
        shutil.copy(wav_fp, self.audio_path)
        shutil.copy(textgrid_fp, self.textgrid_path)
        
        audio, ssr = librosa.load(self.audio_path,sr=args.audio_sr)
        

        # Skip ASR and MFA since we have the files already
    ...
    def test_demo(self, epoch):
        ...
        save NPZ file only (no video rendering)
        '''
        ...
        with torch.no_grad():
            for its, batch_data in enumerate(self.test_loader):
                ...
                # Skip video rendering, just return NPZ path
                logger.info(f"Saved NPZ: {results_npz_file_save_path}")
...
@logger.catch
def gesturelsm_cli(wav_path: str, textgrid_path: str):
    ...
    trainer = BaseTrainer(args, cfg, wav_fp=wav_path, textgrid_fp=textgrid_path)
...
def main():
    parser = argparse.ArgumentParser(description="CLI: WAV + TextGrid -> NPZ (no rendering; config like original)")
    ...
    # parse_known_args -> keeps -c etc. for config.parse_args
    args_cli, remaining = parser.parse_known_args()

    # === Load original Config/Args (incl. -c) ===
    sys.argv = [sys.argv[0]] + remaining  # override sys.argv so config.parse_args() only sees remaining arguments
    
    # Call CLI function
    result = gesturelsm_cli(args_cli.wav, args_cli.textgrid)
    print(result)
